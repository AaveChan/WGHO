...
contract WGHOTest is Test {
    ...
    */
    function testValidPermit() public {
        bytes32 digest = _getPermitDigest(alice, bob, 500e18, 1 days, wGHO.nonces(alice));
    ...
    function testExpiredPermit() public {
        bytes32 digest = _getPermitDigest(alice, bob, 500e18, 1 days, 0);
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(alice_pk, digest);
    ...
    function testInvalidPermitNonce() public {
        bytes32 digest = _getPermitDigest(alice, bob, 500e18, 1 days, wGHO.nonces(alice) + 1); // Addding 1 to nonce makes it invalid
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(alice_pk, digest);
    ...
    function testTransferFromMaxPermit() public {
        bytes32 digest = _getPermitDigest(alice, bob, type(uint256).max, 1 days, wGHO.nonces(alice));
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(alice_pk, digest);
        
        vm.startPrank(alice);
        wGHO.permit(alice, bob, type(uint256).max, 1 days, v, r, s);
        _deposit(500e18);
    ...
    function testTransferFromPermitInvalidAllowance() public {
        bytes32 digest = _getPermitDigest(alice, bob, 400e18, 1 days, wGHO.nonces(alice));
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(alice_pk, digest);
        
        vm.startPrank(alice);
        wGHO.permit(alice, bob, 400e18, 1 days, v, r, s);
        _deposit(500e18);
    ...
    function testTransferFromPermitInvalidBalance() public {
        bytes32 digest = _getPermitDigest(alice, bob, 1000e18, 1 days, wGHO.nonces(alice));
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(alice_pk, digest);
        
        vm.startPrank(alice);
        wGHO.permit(alice, bob, 1000e18, 1 days, v, r, s);
        _deposit(500e18);
